...
function [motionVect, EScomputations, Rood] = motionEstES(imgP, imgI, mbSize, p)
...
for i = 1 : mbSize : row-mbSize+1
    for j = 1 : mbSize : col-mbSize+1
        ...
        % Rood1: SAD of center
        Rood(1,mbCount) = costFuncMAD(imgP(i:i+mbSize-1,j:j+mbSize-1),imgI(i:i+mbSize-1,j:j+mbSize-1),mbSize);
        Rood(2,mbCount) = max(abs(dx-p-1),abs(dy-p-1));
