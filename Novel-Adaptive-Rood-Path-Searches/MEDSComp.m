...
function [MVx, MVy, SAD, DScomputations] = MEDSComp(imgPP, imgII, mbSize, SR, OBT_map)

costs = ones(1, 9) * 65537;
imgP = imgPP(:,:,1);
imgI = imgII(:,:,1);
[row,col] = size(imgI);
MVx = zeros(row/mbSize,col/mbSize);
MVy = MVx;
% we now take effectively log to the base 2 of p
% this will give us the number of steps required

L = floor(log10(SR+1)/log10(2));   
...
for i = 1 : mbSize : row-mbSize+1
    for j = 1 : mbSize : col-mbSize+1
        ...
        x = j;
        y = i;
        edge_map = OBT_map(i:i+mbSize-1,j:j+mbSize-1);
        edge_wei = 1 - edge_map;
        costs(5) = costFuncMAD(imgP(i:i+mbSize-1,j:j+mbSize-1), ...
                                    imgI(i:i+mbSize-1,j:j+mbSize-1),mbSize, edge_wei);
...
%             edge_wei = 1 - edge_map;
            costs(k) = costFuncMAD(imgP(i:i+mbSize-1,j:j+mbSize-1), ...
                  imgI(refBlkVer:refBlkVer+mbSize-1, refBlkHor:refBlkHor+mbSize-1), mbSize, edge_wei);
            computations = computations + 1;
        end
            ...
            if (cornerFlag == 1)
                for k = 1:9
                    ...
                    if ( refBlkHor >= xLast - 1  && refBlkHor <= xLast + 1 ...
                        ...
                        continue;
                    elseif (refBlkHor < j-SR || refBlkHor > j+SR || refBlkVer < i-SR ...
                            || refBlkVer > i+SR)
                        continue;
                    else
                        costs(k) = costFuncMAD(imgP(i:i+mbSize-1,j:j+mbSize-1), ...
                                       imgI(refBlkVer:refBlkVer+mbSize-1, ...
                                          refBlkHor:refBlkHor+mbSize-1), mbSize, edge_wei);
                        computations = computations + 1;
                    end
                end
...
%         vectors(2,mbCount) = x - j;    % col co-ordinate for the vector       
        ...
        aa = floor(i/mbSize);
        bb = floor(j/mbSize);
        MVy(aa+1,bb+1) = y - i; % row co-ordinate for the vector
        MVx(aa+1,bb+1) = x - j; % col co-ordinate for the vector       
        SAD(aa+1,bb+1) = cost;
    ...
    Block_ref   = imgP(i:i+mbSize-1,j:j+mbSize-1);
    refBlkVer = i + MVy(aa+1,bb+1);
    refBlkHor = j + MVx(aa+1,bb+1);
    Block       = imgI(refBlkVer:refBlkVer+mbSize-1,refBlkHor:refBlkHor+mbSize-1);
    
    % Taylor_sol  = Taylor_App(Block, Block_ref);
    [dfx,dfy] = gradient(Block);
    a = sum(dfx(:).^2);  % dx*dx
    b = sum(dfx(:).*dfy(:)); % dx*dy
    d = sum(dfy(:).^2);  % dy*dy
    z = Block_ref - Block;  % diff. of Block & Block_ref
    p = sum(z(:).*dfx(:));
    q = sum(z(:).*dfy(:));
    A = [a b; b d];
    rhs = [p;q];
    ...
    MVy(aa+1,bb+1) = MVy(aa+1,bb+1) + MVy_frac;
    MVx(aa+1,bb+1) = MVx(aa+1,bb+1) + MVx_frac;
...
%         refBlkVer = i + dy;
%         refBlkHor = j + dx;
%         ImCompen(i:i+mbSize-1,j:j+mbSize-1,1:c) = imgII(refBlkVer:refBlkVer+mbSize-1, refBlkHor:refBlkHor+mbSize-1,1:c);
%     
%     end
% end
