...
% In order to avoid calculating the center point of the search again and again 
% we always store the value for it from the previous run. 

% For the first iteration we store this value outside the for loop, but for 
% subsequent iterations we store the cost at the point where we are going 
% to shift our root.
%
% For the NTSS, we find the minimum first in the "far away points" we then find 
% the minimum for the "close up points" we then compare the minimums and 
% which ever is the lowest is where we shift our root of search. 
% If the minimum is the center of the current window then we stop the search. 
% If its one of the immediate close to the center then we will do the second step
% stop. And if its in the far away points, then we go doing about the normal TSS approach
...
% more details in the code below or read the paper/literature
        ...
        stepSize = stepMax;  % stepMax = 2^(L-1);
        computations = computations + 1;

%% 1. This is the calculation of the outer 8 points
...
%% 2. Now find the costs at 8 points right next to the center point
    ...
    end % for i = 1 : mbSize : row-mbSize+1
end % for j = 1 : mbSize : col-mbSize+1
